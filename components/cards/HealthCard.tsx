    ...
    <View className="mx-6 mt-6 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <>
          {state.sample ? (
            <View className="mb-4 items-center rounded-xl bg-red-50 py-5">
              ...
              <Text className="mt-1 text-sm font-medium text-red-400">
                bpm
              </Text>
              <Text className="mt-2 text-xs text-gray-400">
                {formatDate(state.sample.startDate)} at{" "}
                {formatTime(state.sample.startDate)}
              </Text>
            </View>
          ) : (
      ...
      {state.phase === "error" && (
        <>
          ...
          <Pressable
            onPress={() => setState({ phase: "not_connected" })}
            className="items-center rounded-xl bg-gray-100 py-3"
          >
